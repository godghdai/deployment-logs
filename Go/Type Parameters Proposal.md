- [Type Parameters Proposal](#type-parameters-proposal)
  * [Status](#status)
  * [Abstract](#abstract)
  ...
  * [Design](#design)
    ...
    + [Operations permitted for any type](#operations-permitted-for-any-type)
    + [Defining constraints](#defining-constraints)
    + [The `any` constraint](#the-any-constraint)
    + [Using a constraint](#using-a-constraint)
    + [Multiple type parameters](#multiple-type-parameters)
    + [Generic types](#generic-types)
    + [Methods may not take additional type arguments](#methods-may-not-take-additional-type-arguments)
    + [Operators](#operators)
      - [Type sets](#type-sets)
      - [Type sets of constraints](#type-sets-of-constraints)
      - [Constraint elements](#constraint-elements)
        * [Arbitrary type constraint element](#arbitrary-type-constraint-element)
        * [Approximation constraint element](#approximation-constraint-element)
        * [Union constraint element](#union-constraint-element)
      - [Operations based on type sets](#operations-based-on-type-sets)
      - [Comparable types in constraints](#comparable-types-in-constraints)
    + [Mutually referencing type parameters](#mutually-referencing-type-parameters)
    + [Type inference](#type-inference)
      - [Type unification](#type-unification)
      - [Function argument type inference](#function-argument-type-inference)
      - [Constraint type inference](#constraint-type-inference)
        * [Element constraint example](#element-constraint-example)
        * [Pointer method example](#pointer-method-example)
        * [Constraints apply even after constraint type inference](#constraints-apply-even-after-constraint-type-inference)
    + [Using types that refer to themselves in constraints](#using-types-that-refer-to-themselves-in-constraints)
    + [Values of type parameters are not boxed](#values-of-type-parameters-are-not-boxed)
    + [More on type sets](#more-on-type-sets)
      - [Both elements and methods in constraints](#both-elements-and-methods-in-constraints)
      - [Composite types in constraints](#composite-types-in-constraints)
      - [Type parameters in type sets](#type-parameters-in-type-sets)
      - [Type conversions](#type-conversions)
      - [Untyped constants](#untyped-constants)
      - [Type sets of embedded constraints](#type-sets-of-embedded-constraints)
      - [Interface types in union elements](#interface-types-in-union-elements)
      - [Empty type sets](#empty-type-sets)
      - [General notes on type sets](#general-notes-on-type-sets)
    ...
    + [Summary](#summary)
      ...
      - [Issues](#issues)
        ...
        * [No way to express convertibility](#no-way-to-express-convertibility)
        * [No parameterized methods](#no-parameterized-methods)
        * [No way to require pointer methods](#no-way-to-require-pointer-methods)
        * [No association between float and complex](#no-association-between-float-and-complex)
      - [Discarded ideas](#discarded-ideas)
        * [What happened to contracts?](#what-happened-to-contracts)
        * [Why not use methods instead of type sets?](#why-not-use-methods-instead-of-type-sets)
        * [Why not put type parameters on packages?](#why-not-put-type-parameters-on-packages)
        * [Why not use the syntax `F` like C++ and Java?](#why-not-use-the-syntax-f-like-c-and-java)
        * [Why not use the syntax `F(T)`?](#why-not-use-the-syntax-ft)
        * [Why not use `F«T»`?](#why-not-use-f%C2%ABt%C2%BB)
        * [Why not define constraints in a builtin package?](#why-not-define-constraints-in-a-builtin-package)
        * [Why not permit type assertions on values whose type is a type parameter?](#why-not-permit-type-assertions-on-values-whose-type-is-a-type-parameter)
      - [Comparison with Java](#comparison-with-java)
      - [Comparison with C++](#comparison-with-c)
      - [Comparison with Rust](#comparison-with-rust)
  * [Examples](#examples)
    + [Map/Reduce/Filter](#mapreducefilter)
    + [Map keys](#map-keys)
    + [Sets](#sets)
    + [Sort](#sort)
    + [Channels](#channels)
  ...
  * [Appendix](#appendix)
    ...
    + [Instantiating a function](#instantiating-a-function)
    + [Embedded type parameter](#embedded-type-parameter)
    + [Embedded type parameter methods](#embedded-type-parameter-methods)
    + [Embedded instantiated type](#embedded-instantiated-type)
    + [Generic types as type switch cases](#generic-types-as-type-switch-cases)
    + [Method sets of constraint elements](#method-sets-of-constraint-elements)
    + [Permitting constraints as ordinary interface types](#permitting-constraints-as-ordinary-interface-types)
    + [Type inference for composite literals](#type-inference-for-composite-literals)
    + [Type inference for generic function arguments](#type-inference-for-generic-function-arguments)
    + [Reflection on type arguments](#reflection-on-type-arguments)
...
In the following sections we work through each of these language changes in great detail. You may prefer to skip ahead to the [examples](https://go.googlesource.com/proposal/+/refs/heads/master/design/43651-type-parameters.md#Examples) to see what generic code written to this design will look like in practice.
...
Here is a function that prints out each element of a slice, where the element type of the slice, here called `T`, is unknown. This is a trivial example of the kind of function we want to permit in order to support generic programming. (Later we'll also discuss [generic types](https://go.googlesource.com/proposal/+/refs/heads/master/design/43651-type-parameters.md#Generic-types)).
...
Since `Print` has a type parameter, any call of `Print` must provide a type argument. Later we will see how this type argument can usually be deduced from the non-type argument, by using [type inference](https://go.googlesource.com/proposal/+/refs/heads/master/design/43651-type-parameters.md#Type-inference). For now, we'll pass the type argument explicitly. Type arguments are passed much like type parameters are declared: as a separate list of arguments. As with the type parameter list, the list of type arguments uses square brackets.
...
Therefore, in this design, constraints are simply interface types. Satisfying a constraint means implementing the interface type. (Later we'll restate this in order to define constraints for operations other than method calls, such as [binary operators](https://go.googlesource.com/proposal/+/refs/heads/master/design/43651-type-parameters.md#Operators)).
...
There is more discussion of this in [the issues section](https://go.googlesource.com/proposal/+/refs/heads/master/design/43651-type-parameters.md#No-parameterized-methods).
...
It's possible to use `comparable` to produce a constraint that can not be satisfied by any type. See also the [discussion of empty type sets below](https://go.googlesource.com/proposal/+/refs/heads/master/design/43651-type-parameters.md#Empty-type-sets).
...
To see how it works, let's go back to [the example](https://go.googlesource.com/proposal/+/refs/heads/master/design/43651-type-parameters.md#Type-parameters) of a call to the simple `Print` function:
...
- Apply function type inference using the default types of any remainint untyped arguments.
- Apply constraint type inference again.
...
We then unify each type parameter with a structural constraint with the single type in that constraint's type set. In this case the structural constraint is `SC[E]` which has the single type `[]E`, so we unify `S` with `[]E`. Since we already have a mapping for `S`, we then unify `[]E` with `MySlice`. As `MySlice` is defined as `[]int`, that associates `E` with `int`. We now have:
...
Let's return now to type sets to cover some less important details that are still worth noting. These are not additional rules or concepts, but are consequences of how type sets work.
...
We showed other examples of this earlier in the discussion of [constraint type inference](https://go.googlesource.com/proposal/+/refs/heads/master/design/43651-type-parameters.md#Constraint-type-inference).
...
The conversion from type `T2` to type `T1` is invalid, as there is no constraint on either type that permits the conversion. Worse, there is no way to write such a constraint in general. In the particular case where both `T1` and `T2` have limited type sets this function can be written as described earlier when discussing [type conversions using type sets](https://go.googlesource.com/proposal/+/refs/heads/master/design/43651-type-parameters.md#Type-conversions). But, for example, there is no way to write a constraint for the case in which `T1` is an interface type and `T2` is a type that implements that interface.
...
For example, consider a variant of the `Stringify` example we [showed earlier](https://go.googlesource.com/proposal/+/refs/heads/master/design/43651-type-parameters.md#Using-a-constraint).
